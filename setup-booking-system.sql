...
-- Run this script directly in your Supabase SQL editor to set up the booking system
...
BEGIN
  ...
  -- Debug: Log the working hours query
  RAISE NOTICE 'SQL: Looking for day_of_week = % for date %', v_day_of_week, p_date;

  -- If no working hours set for this day, return empty
  IF NOT FOUND THEN
    RAISE NOTICE 'SQL: No working hours found for day_of_week = %', v_day_of_week;
    RETURN;
  END IF;

  RAISE NOTICE 'SQL: Found working hours for day_of_week = %, start_time = %, end_time = %', v_day_of_week, v_working_hours.start_time, v_working_hours.end_time;
