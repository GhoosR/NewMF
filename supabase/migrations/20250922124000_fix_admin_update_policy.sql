...
DROP POLICY IF EXISTS "admin_update_group_chats" ON conversations;
...
CREATE POLICY "admin_group_updates"
ON conversations
FOR UPDATE
USING (
  type = 'group' AND
...
CREATE POLICY "participant_updates"
ON conversations
FOR UPDATE
USING (
  auth.uid() = ANY(participant_ids)
...
WITH CHECK (
  ...
  (
    -- Allow updating last_viewed fields
    (
      auth.uid() = last_viewed_by
    ) OR
    -- Allow admin to update group chat
    (
      type = 'group' AND
      auth.uid() = admin_id
    ) OR
    -- Allow participants to update direct chats
    (
      type = 'direct'
    )
  )
