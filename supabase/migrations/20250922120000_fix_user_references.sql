-- First drop the existing junction table if it exists
DROP TABLE IF EXISTS conversation_participants;

-- Create a function to validate participant_ids against users
...
BEGIN
  ...
  IF NOT (
    SELECT bool_and(EXISTS (
      SELECT 1 FROM users WHERE id = participant_id
    ))
