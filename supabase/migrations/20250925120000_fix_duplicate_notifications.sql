-- Fix duplicate notification issue by ensuring only one trigger exists
-- and properly handles group vs direct message notifications

-- Drop ALL existing notification triggers and functions
...
-- Create a single, comprehensive notification function
...
-- Remove duplicate notifications keeping only the most recent one per conversation per user
WITH duplicates AS (
  SELECT 
    id,
    ROW_NUMBER() OVER (
