...
Deno.serve(async (req) => {
  ...
  try {
    console.log('RevenueCat webhook final function started')
    
    // Initialize Supabase client with service role key for webhook operations
    const supabaseUrl = 'https://afvltpqnhmaxanirwnqz.supabase.co'
    ...
    switch (event.type) {
      ...
      case 'BILLING_ISSUE':
      case 'PRODUCT_CHANGE':
        console.log('Processing subscription end event:', event.type)
        ...
        console.log('Successfully processed subscription end event')
        break
